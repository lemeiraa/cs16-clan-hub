...
function LojaPage() {
  ...
  return (
    <section className="container mx-auto px-4 py-12">
      <div className="mb-8">
        <p className="text-xs uppercase tracking-[0.3em] text-accent font-semibold">Apoie a comunidade</p>
        <h1 className="font-display text-4xl font-bold mt-1">Loja Oficial</h1>
        <p className="text-muted-foreground mt-2 max-w-2xl">
          Pagamento via PIX ou WhatsApp. Após confirmação, um admin aplica o benefício no servidor em até 24 horas.
        </p>
      </div>
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr,400px]">
      <div className="rounded-xl border border-border bg-card p-6 shadow-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              R$ {Number(ammo.price_per_1000).toFixed(2)} a cada 1.000 ammo packs · máx {ammo.max_qty.toLocaleString("pt-BR")} por compra
            </p>
          </div>
        </div>
        <div className="mt-6">
        ...
        <div className="mt-6 rounded-lg bg-gradient-brand p-5 text-brand-foreground">
          <p className="text-xs uppercase tracking-wider opacity-80">Total a pagar</p>
          <p className="font-display text-4xl font-bold mt-1">R$ {price.toFixed(2).replace(".", ",")}</p>
          <p className="text-sm mt-1 opacity-80">{safeQty.toLocaleString("pt-BR")} ammo packs</p>
        </div>
      </div>
      <CheckoutForm
        productType="ammo_packs" ammoPacks={safeQty} amount={price} label="Comprar Ammo Packs"
...
function CheckoutForm({
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.nick || !form.contact_email) { toast.error("Preencha nick e email."); return; }
    if (!form.contact_whatsapp || form.contact_whatsapp.replace(/\D/g, "").length < 10) {
      toast.error("WhatsApp é obrigatório para concluir a compra."); return;
    }
    setLoading(true);
    try {
      ...
      toast.error("Erro ao gerar PIX", { description: err instanceof Error ? err.message : String(err) });
    } finally { setLoading(false); }
  };

  return (
    <form onSubmit={submit} className="mt-5 space-y-3 rounded-lg border border-accent/40 bg-secondary/30 p-4">
      <Field label="Nick no jogo *" value={form.nick} onChange={(v) => setForm({ ...form, nick: v })} />
      <Field label="Email *" type="email" value={form.contact_email} onChange={(v) => setForm({ ...form, contact_email: v })} />
      <Field label="WhatsApp *" value={form.contact_whatsapp} onChange={(v) => setForm({ ...form, contact_whatsapp: v })} />
      <Field label="SteamID (opcional)" value={form.steamid} onChange={(v) => setForm({ ...form, steamid: v })} />
      {!forcedServerSlug && (
        <div>
      ...
      <div className="text-sm">
        Total: <span className="font-display text-lg font-bold text-gradient">R$ {amount.toFixed(2).replace(".", ",")}</span>
      </div>

      {paymentMethod === "pix" && methods.pix ? (
        <>
          <div className="flex gap-2">
            <button type="button" onClick={() => setOpen(false)} className="px-3 py-2 text-sm rounded-md border border-border hover:bg-secondary transition">Cancelar</button>
          ...
          <p className="text-[11px] text-muted-foreground">Pagamento via PIX (Mercado Pago). Confirmação automática, benefício aplicado em até 24h.</p>
        </>
      ) : (
        <>
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground">Fale com um admin no WhatsApp para combinar o pagamento:</p>
            {waAdmins.map((a) => {
              ...
              return (
                <a key={a.phone} href={canSend ? `https://wa.me/${a.phone}?text=${waMessage}` : undefined}
                  ...
                  onClick={(e) => { if (!canSend) { e.preventDefault(); toast.error("Preencha nick, email e WhatsApp antes de continuar."); } }}
          ...
          <button type="button" onClick={() => setOpen(false)} className="w-full px-3 py-2 text-sm rounded-md border border-border hover:bg-secondary transition">Cancelar</button>
          <p className="text-[11px] text-muted-foreground">O admin confirma o pagamento manualmente e aplica o benefício no servidor em até 24h.</p>
        </>
      )}
    </form>
  );
  ...
  return (
    <div className="mt-5 rounded-lg border border-accent/40 bg-secondary/30 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h4 className="font-display text-lg font-bold">{paid ? "Pagamento confirmado!" : "Pague com PIX"}</h4>
      ...
      {paid ? (
        <div className="text-center py-6">
          ...
          <p className="mt-3 text-sm">Recebemos seu pagamento. Um admin vai aplicar o benefício no servidor em até 24h.</p>
          <p className="mt-2 text-xs text-muted-foreground font-mono">Pedido: {pix.orderId.slice(0, 8)}</p>
        </div>
      ) : (
        <>
          {pix.qrCodeBase64 && (
          ...
          <div className="text-sm">Total: <span className="font-display text-lg font-bold text-gradient">R$ {pix.amount.toFixed(2).replace(".", ",")}</span></div>
          {pix.ticketUrl && (
      ...
      <button type="button" onClick={onClose} className="w-full px-3 py-2 text-sm rounded-md border border-border hover:bg-secondary transition">Fechar</button>
    </div>
  );
}
